...
def get_other_config(seed=42, hidden_dim=64, num_layers=5):
    ...
    wandb.project = "Burgers-WeightingSweep"
    wandb.tag = None
    ...
    arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 1, "embed_dim": hidden_dim})
    arch.periodicity = ml_collections.ConfigDict(
        {"period": (jnp.pi,), "axis": (1,)}
        )
    arch.reparam = None
    ...
    config.optim.first_order_steps = 70_000
...
def get_config(weighting_scheme, seed):
    config = get_other_config(seed=seed)

    config.wandb.name = "{weighting_scheme}_Soap_{seed}_{hidden_dim}_{num_layers}".format(weighting_scheme=weighting_scheme, seed=seed, hidden_dim=config.arch.hidden_dim, num_layers=config.arch.num_layers)
