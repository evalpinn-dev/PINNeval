...
class KDV(PINN):
    def __init__(self, config, axes_dict, initial_dict, pde_params_dict=None):
        super().__init__(config, axes_dict=axes_dict, initial_dict=initial_dict, pde_params_dict=pde_params_dict)
    ...
    def r_net(self, params, t, x):
        ...
        # u, u_t, u_x from one reverse pass
        u, u_grad = value_and_grad(u_z)(z)

        # u_xxx from jet of order 3 along x
        v1 = jnp.array([0.0, 1.0], dtype=z.dtype)
        zero = jnp.zeros_like(v1)
        _, (_, _, u_xxx) = jet(u_z, (z,), [[v1, zero, zero]])

        return u_grad[0] + u * u_grad[1] + 0.022**2 * u_xxx
