...
def get_other_config(seed=42):
    ...
    wandb.project = "Poisson-OptSweep"
    wandb.tag = None
    ...
    weighting.init_weights = ml_collections.ConfigDict({"u_bc": 20.0, "r": 1.0})
...
def get_config(opt, seed, dim=5):
    ...
    if opt == "Adam":
        ...
        optim.first_order_steps = 40_000
    ...
    if opt in ["Soap_SSBroyden2"]:
        ...
        arch.hidden_dim = 64
        arch.fourier_emb = ml_collections.ConfigDict(
            {"embed_scale": 1.0, "embed_dim": 64}
        )
        optim.first_order_steps = 2_000
        optim.second_order_steps = 10_000

    config.wandb.name = "{opt}_{nn}_{hidden_dim}_{num_layers}_{seed}_dim_{dim}".format(
        nn=arch.arch_name,
        opt=opt,
        seed=seed,
