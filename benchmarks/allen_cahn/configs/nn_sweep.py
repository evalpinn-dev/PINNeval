...
def get_config(nn_arch, seed):
    ...
    if nn_arch == "MLP_small":
        ...
        arch.num_layers = 3
        arch.hidden_dim = 24
        arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 1, "embed_dim": 24})
    
    elif nn_arch == "PirateNet_med":
        arch.arch_name = "PirateNet"
        arch.num_layers = 3
        arch.hidden_dim = 48
        arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 1, "embed_dim": 48})
    
    elif nn_arch == "PirateNet_large":
        arch.arch_name = "PirateNet"
        arch.num_layers = 3
        arch.hidden_dim = 96
        arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 1, "embed_dim": 96})

    config.wandb.name = "{nn}_Soap_{seed}_{hidden_dim}_{num_layers}".format(nn=nn_arch, seed=seed, hidden_dim=config.arch.hidden_dim, num_layers=config.arch.num_layers)
